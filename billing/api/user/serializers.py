...
class RegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = NewUSER
        fields = ['email', 'password', 'password2',"user_name","first_name","role_id"]
        extra_kwargs = {
            'password': {'write_only': True}
        }

    def save(self):
        user = NewUSER(email=self.validated_data['email'], user_name=self.validated_data['user_name'],first_name=self.validated_data['first_name'],role_id = self.validated_data["role_id"])
